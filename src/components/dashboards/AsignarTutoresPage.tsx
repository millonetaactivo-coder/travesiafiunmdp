...
export const AsignarTutoresPage = () => {
  ...
  const [viewAllCarreras, setViewAllCarreras] = useState(false);
  ...
  const fetchData = useCallback(async () => {
    if (!viewAllCarreras && !carreraId) return;
    ...
    try {
      ...
      const filters: StudentFilters = {
        ...
        hasTutor: viewAllCarreras ? false : (filterTutor === 'con' ? true : filterTutor === 'sin' ? false : undefined),
      };

      const [estResult, tutoresResult, asignResult] = await Promise.all([
        getEstudiantesFiltrados(viewAllCarreras ? null : carreraId, filters),
        getTutores(),
        getAsignaciones(),
      ]);
  ...
  }, [carreraId, filterRiesgo, filterAnio, filterTutor, viewAllCarreras]);
  ...
  // When global view is on, filterTutor is forced to 'sin' and locked — don't count it
  // as a user-controlled filter in the badge or clear it.
  const activeFilterCount = viewAllCarreras
    ? [filterAnio, filterRiesgo].filter(Boolean).length
    : [filterAnio, filterRiesgo, filterTutor].filter(Boolean).length;
  ...
  // Toggle "all careers, sin tutor" view. When ON, overrides the global career scope
  // and forces the "Estado tutor" filter to "sin". When OFF, resets "Estado tutor" to "Todos".
  const handleToggleGlobalView = () => {
    if (viewAllCarreras) {
      setViewAllCarreras(false);
      setFilterTutor('');
    } else {
      setViewAllCarreras(true);
      setFilterTutor('sin');
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <header className="flex justify-between items-start mb-8">
        <div>
          ...
          <p className="text-sm text-slate-500 font-sans mt-2">
            {viewAllCarreras
              ? `${filteredEstudiantes.length} estudiante${filteredEstudiantes.length !== 1 ? 's' : ''} sin tutor (todas las carreras) · ${tutores.length} tutor${tutores.length !== 1 ? 'es' : ''}`
      ...
      {/* Global view banner — only when toggle is ON */}
      <AnimatePresence>
        {viewAllCarreras && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -8 }}
            className="bg-blue-500/10 border border-blue-500/20 rounded-2xl px-4 py-3 backdrop-blur-md flex items-center gap-3"
          >
            <Globe className="w-4 h-4 text-blue-400 shrink-0" />
            <p className="text-xs font-sans text-blue-300">
              Vista global activa — el selector de carrera de arriba no aplica.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <div className="flex items-center justify-between mb-3 gap-2">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={cn(
                "flex items-center gap-1.5 text-xs font-sans rounded-lg px-2.5 py-1.5 transition-colors border",
                viewAllCarreras
                  ? "bg-blue-500/15 border-blue-500/30 text-blue-300"
                  : "bg-white/[0.03] border-white/[0.08] text-slate-400 hover:text-white hover:border-white/[0.15]"
              )}
              title={viewAllCarreras ? "Salir de la vista global" : "Ver todas las carreras (solo sin tutor)"}
            >
              <Globe className="w-3.5 h-3.5" />
              {viewAllCarreras ? 'Vista global: sin tutor' : 'Todas las carreras'}
            </button>
          </div>
          {activeFilterCount > 0 && (
          ...
          {showFilters && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-white/[0.05]">
                ...
                <div className={viewAllCarreras ? "opacity-50 pointer-events-none" : undefined}>
                  <label className="block text-[10px] text-slate-500 font-sans uppercase tracking-wider mb-1">
                    Estado tutor{viewAllCarreras ? ' (bloqueado)' : ''}
                  </label>
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[10px] text-slate-500 font-sans truncate">
                            {est.email}
                            {viewAllCarreras && est.carrera_nombre ? ` · ${est.carrera_nombre}` : ''}
                          </p>
                        </div>
                      </div>
